...
video-trim 视频裁剪的意思。开发这个工具的初衷是，在手机上下载的视频(你懂的)，总是有一些开头一样的背景，导致手机打开相册的时候看到的所有都是一样的封面图，想找个需要的视频，找半天都找不到，这不能忍。

最初我使用手机上相册自带的剪辑功能，盲猜是因为要编码所以慢的要死。我是想要快速的将指定时间段的剪切掉即可，使用手机原生剪辑这太浪费时间了，也想过自己在手机上使用 ffmpeg 来，使用命令行来实现，但是要 root 权限，还是放弃了。

所以就将这个功能转移到了自己局域网的工作电脑了，性能跟得上，节约时间。就套壳一下 ffmpeg，手机(PC也可以)使用浏览器访问即可实现使用。
...
- 内网上传下载飞快。
- 只是裁剪不重新编码，使用 ffmpeg 的 copy 参数即可，飞快。
